...
class MeasureCollection extends BaseCollection {
  constructor() {
    super('Measures', new SimpleSchema({
      ...
      notifyUsers: { type: Array, defaultValue: [] },
      'notifyUsers.$': String,
    }));
  }

  define({ year, measureType, measureNumber, officeType, lastUpdated, code, measurePdfUrl, measureArchiveUrl, measureTitle, reportTitle, bitAppropriation, description, status, introducer, currentReferral, companion, notifyUsers }) {
    ...
    const docID = this._collection.insert({ year, measureType, measureNumber, officeType,
      lastUpdated, code, measurePdfUrl, measureArchiveUrl, measureTitle, reportTitle, bitAppropriation, description, status, introducer, currentReferral, companion, notifyUsers });
    return docID;
  }

  update(docID, { lastUpdated, code, measurePdfUrl, officeType, measureArchiveUrl, measureTitle, reportTitle, bitAppropriation, description, status, introducer, currentReferral, companion, notifyUsers }) {
    const updateData = {};
    if (lastUpdated) {
    ...
    if (notifyUsers) {
      updateData.notifyUsers = notifyUsers;
    }
  ...
  dumpOne(docID) {
    ...
    const notifyUsers = doc.notifyUsers;
    return { year, measureType, officeType, measureNumber, lastUpdated, code, measurePdfUrl, measureArchiveUrl, measureTitle, reportTitle, bitAppropriation, description, status, introducer, currentReferral, companion, notifyUsers };
  }
}
