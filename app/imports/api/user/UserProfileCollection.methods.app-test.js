...
if (Meteor.isClient) {
  describe('UserProfileCollection Meteor Methods', function testSuite() {
    it('Can define, update, and removeIt', async function test1() {
      ...
      definitionData.employeeID = 'dummyID';
      ...
      expect(doc.employeeID).to.equal(definitionData.employeeID);
      expect(doc.newAccount).to.equal(definitionData.newAccount);
      const updateData = {};
      updateData.id = docID;
      updateData.firstName = faker.name.firstName();
      updateData.lastName = faker.name.lastName();
      updateData.employeeID = '0';
      updateData.newAccount = false;
